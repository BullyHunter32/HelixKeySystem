...
function SWEP:PrimaryAttack()
	...
	if (IsValid(entity) and
		(
			(entity:IsDoor() and (entity:CheckDoorAccess(self.Owner) or entity.KeyRequired)) or
			(entity:IsVehicle() and entity.CPPIGetOwner and entity:CPPIGetOwner() == self.Owner)
		)
	) then
        local pPlayer = self:GetOwner()
        local char = pPlayer:GetCharacter()
        if char and entity.KeyRequired then
            print("Key required")
            local inventory = char:GetInventory()
            local tKeys = inventory:GetItemsByUniqueID("housekey", false)
            for k,v in ipairs(tKeys) do
                print("Comparing",v.data.PropertyID, entity.KeyRequired, v.data.PropertyID == entity.KeyRequired)
                if tonumber(v.data.PropertyID) == tonumber(entity.KeyRequired) then
                    print("Found the key! ", v)
                    pPlayer:SetAction("@locking", time, function()
                        self:ToggleLock(entity, true)
                    end)
                    break
                end
            end
        elseif not entity.KeyRequired then
            pPlayer:SetAction("@locking", time, function()
                self:ToggleLock(entity, true)
            end)
        end
    ...
    (
        ...
        if char and entity.KeyRequired then
            ...
            for k,v in ipairs(tKeys) do
                ...
                if tonumber(v.data.PropertyID) == tonumber(entity.KeyRequired) then
                    print("Found the key! ", v)
                    pPlayer:SetAction("@unlocking", time, function()
